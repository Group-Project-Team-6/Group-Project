	...
	for (auto i = first; i != last; ++i) {
		...
		for (auto j = i + 1; j != last; ++j) {
			...
			if (CollisionDetection::ObjectIntersection(*i, *j, info)) {
				...
				if (info.a->GetPhysicsObject()->GetCollisionResponces() == true && info.b->GetPhysicsObject()->GetCollisionResponces() == true) {
					ImpulseResolveCollision(*info.a, *info.b, info.point);
					allCollisions.insert(info);
					if (info.a->GetLayer() == 0 && info.b->GetLayer() == 0) {
						SetButtonState(true);
					}
					if ((info.a->GetLayer() == 2 && info.b->GetLayer() == 4) || (info.b->GetLayer() == 2 && info.a->GetLayer() == 4)) {
						SetgoalState(true);
					}
					if ((info.a->GetName() == "PlayerSphere" && info.b->GetName() == "EnemySphere" || (info.b->GetName() == "PlayerSphere" && info.a->GetName() == "EnemySphere"))) {
						SetCatchState(true);
					}
					if ((info.a->GetName() == "PlayerSphere" && info.b->GetName() == "MazeGoal" || (info.b->GetName() == "PlayerSphere" && info.a->GetName() == "MazeGoal"))) {
						SetReachMazeGoal(true);
					}
					if ((info.a->GetLayer() == 2 && info.b->GetLayer() == 5) || (info.b->GetLayer() == 2 && info.a->GetLayer() == 5)) {
						SetReset(true);
					}
				}
				else {
					std::cout << "Collied with un-colliedable object." << std::endl;
					if ((info.a->GetLayer() == 2 && info.b->GetLayer() == 3) || (info.b->GetLayer() == 2 && info.a->GetLayer() == 3)) {
...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	float j = (-(1.0f + (cRestitution * physA->GetElasticity() * physB->GetElasticity())) * impulseForce) / (totalMass + angularEffect);
